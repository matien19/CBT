                  ...
                  <div class="col-lg-12">
                    <div class="card">
                      <div class="card-header" style="background-color:#597445">
                        <font color="ffffff">
                          <h3 class="card-title"><i class="nav-icon fas fa-user"></i>Data Guru</h3>
                      </div>
                      </font>
                      <!-- /.card-header -->
                      <div class="card-body">
                        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#modal-tambahdata"
                          style="background-color:#86090f">
                        ...
                        <a href="reset.php" class="btn btn-danger"
                          onclick="return confirm('Anda akan menghapus seluruh data Guru ?')">
                            ...
                            if (mysqli_num_rows($sql_guru) > 0) {
                              while ($data = mysqli_fetch_array($sql_guru)) {
                                ...
                                <tr>
                                  ...
                                  <td>
                                    <center>
                                      <a href="resetpw.php?nip=<?= $data['nip']; ?>" class="btn btn-warning btn-sm"
                                        onclick="return confirm('Anda akan mereset password Dosen [ <?= $data['nip'] . ' - ' . $data['nama']; ?> ]?')">
                                        <i class="fas fa-edit"></i>
                                        Reset Password
                                      </a>

                                      <button class="btn btn-primary btn-sm" data-toggle="modal"
                                        data-target="#modal-editdata" data-nip="<?= $data['nip']; ?>"
                                        data-nama="<?= $data['nama']; ?>">
                                        <i class="fas fa-edit"></i>
                                        Edit
                                      </button>
                                      ...
                                      <button class="btn btn-success btn-sm" data-toggle="modal"
                                        data-target="#modal-editmapel" data-nip="<?= $data['nip']; ?>"
                                        data-nama="<?= $data['nama']; ?>">
                                        <i class="fas fa-clipboard-list"></i>
                                        Input Mapel
                                      </button>

                                      <a href="delete.php?nip=<?= $data['nip']; ?>" class="btn btn-danger btn-sm"
                                        onclick="return confirm('Anda akan menghapus Data Guru [ <?= $data['nip'] . ' - ' . $data['nama']; ?> ] ?')">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form class="form-horizontal" action="addmapel.php" method="POST">
          ...
          <div class="modal-footer pull-right">
            <button type="submit" class="btn btn-warning" name="addmapel" style="background-color:#FCDC2A"><i
                class="nav-icon fas fa-edit"></i>Input Data</button>
        </form>
      </div>
    </div>
    <!-- /.modal-content -->
